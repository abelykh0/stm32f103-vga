...
static const uint8_t* font = font8x8;
 ...
 */
void Vga::print_char(uint8_t x, uint8_t y, unsigned char c, uint8_t attribute)
{
    draw_char(Vga::font, x * 8, y * 8, c);
    Vga::VideoMemoryAttributes[y * HSIZE_CHARS + x] = attribute;
}
...
void Vga::print(uint8_t c, uint8_t attribute)
{
    switch (c)
    {
    case '\0': //null
    ...
    case 8: //backspace
        if (cursor_x > 0)
        {
            print_char(cursor_x - 1, cursor_y, ' ', attribute);
    ...
    default:
        ...
        uint8_t y = cursor_y;
        cursorNext();
        print_char(x, y, c, attribute);
    }
}

void Vga::print(const char *str, uint8_t attribute)
{
    while (*str)
    {
        print(*str++, attribute);
